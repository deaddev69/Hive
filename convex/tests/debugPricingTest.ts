...
import { calculateProductPricing, getPlatformSettings } from "../pricingService";

export const runPricingTests = mutation({
  args: {},
  handler: async (ctx) => {
    console.log("=== STARTING HIVE ALL-IN PRICING ENGINE TESTS ===");

    const settings = await getPlatformSettings(ctx);

    // Test A: Base Price ₹899
    console.log("Test A: Base Price ₹899...");
    const pricingA = calculateProductPricing(899, null, settings);
    ...
    // Charm rounded: Math.ceil(1080.2276 / 10) * 10 - 1 = 1089 (Wait, let's verify if pricingA.customerPrice matches 1089)
    ...
    console.log("Test B: Base Price ₹2500...");
    const pricingB = calculateProductPricing(2500, null, settings);
    // Base: 2500
    ...
    console.log("=== ALL PRICING INTEGRITY TESTS PASSED ===");
    return { success: true };
  }
});
