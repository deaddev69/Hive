...
export default function AdminBlogManagementPage() {
  ...
  const seedMutation = useMutation(api.blogs.seedInitialBlogPosts);
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      alert(`Successfully synced ${res.insertedCount} articles into the database!`);
    } catch (err: any) {
      console.error("Seed error:", err);
      alert("Error seeding articles: " + (err.message || "Unknown error"));
    } finally {
      setIsSeeding(false);
    }
  };
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden">
        {posts === undefined ? (
          ...
          <div className="text-center py-16 px-4">
            <div className="w-14 h-14 rounded-3xl bg-amber-50 text-amber-600 flex items-center justify-center mx-auto mb-3 shadow-inner">
              <BookOpen className="w-7 h-7" />
            </div>
            <h3 className="text-lg font-serif font-black text-slate-800">
              {searchQuery ? "No Articles Found" : "Your Database Is Ready For Articles"}
            </h3>
            <p className="text-xs text-slate-500 max-w-md mx-auto mt-1 mb-6 leading-relaxed">
              {searchQuery
                ? "No blog posts match your search query."
                : "Sync the 6 foundational Kochi editorial guides into your Convex database with 1 click, or start writing a fresh article."}
            </p>
            <div className="flex flex-wrap items-center justify-center gap-3">
              {!searchQuery && (
                <button
                  ...
                  onClick={handleSeed}
                  disabled={isSeeding}
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold text-xs shadow-md transition-all disabled:opacity-50"
                >
                  {isSeeding ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                  Seed 6 Editorial Guides into Database
                </button>
              )}
              <Link
                href="/admin/blog/new"
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs shadow-md transition-all"
              >
                <Plus className="w-4 h-4" /> Compose Custom Article
              </Link>
            </div>
          </div>
        ) : (
