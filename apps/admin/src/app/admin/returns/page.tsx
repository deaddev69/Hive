...
export default function AdminReturnsAndCoupons() {
  ...
  const failedRefunds = useQuery(api.payments.listFailedRefundsAdmin, {});
  ...
  const retryRefund = useMutation(api.payments.retryFailedRefundAdmin);
...
What does the customer want instead?`
  ...
  // A failed refund means the order says "refunded" but the customer never got
  // the money. Re-queues it for the next drain cycle.
  const handleRetryRefund = async (refundQueueId: Id<"refundQueue">, amountPaise: number) => {
    if (
      !window.confirm(
        `Retry this refund of ₹${(amountPaise / 100).toFixed(2)}?

It will be attempted again on the next refund cycle.`
      )
    )
      return;
    setBusy(refundQueueId);
    try {
      await retryRefund({ refundQueueId });
      alert("Refund re-queued. It will be retried within 15 minutes.");
    } catch (err: any) {
      alert(err.message || "Failed to re-queue the refund.");
    } finally {
      setBusy(null);
    }
  };
    ...
    <div className="flex flex-col gap-6 pb-24">
      ...
      {tab === "recovery" && (
        <div className="space-y-4">
          {failedRefunds && failedRefunds.length > 0 && (
            <div className="rounded-2xl border border-rose-300 bg-rose-50 p-4">
              ...
              <p className="text-[11px] text-rose-800 mb-3">
                These orders show as refunded, but the money never reached the customer.
                Fix the cause, then retry.
              </p>
              <div className="space-y-2">
                {failedRefunds.map((f: any) => (
                  <div
                    key={f._id}
                    className="rounded-xl border border-rose-200 bg-white p-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"
                  >
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-slate-900">
                        {f.orderNumber ?? "—"} · {rupees(f.amountPaise)}
                      </p>
                      <p className="text-[11px] text-slate-600 mt-0.5 break-words">
                        {f.lastError ? String(f.lastError).slice(0, 160) : "No error recorded"}
                      </p>
                    </div>
                    <button
                      type="button"
                      disabled={busy === f._id}
                      onClick={() => handleRetryRefund(f._id, f.amountPaise)}
                      className="px-3 py-2 rounded-lg bg-rose-600 hover:bg-rose-700 text-white text-[11px] font-bold transition-all disabled:opacity-60 cursor-pointer whitespace-nowrap shrink-0"
                    >
                      Retry refund
                    </button>
                  </div>
