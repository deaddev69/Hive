...
export default function BoutiqueProfile() {
  ...
  return (
    <div className="flex flex-col gap-8 text-left">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col gap-5 shadow-2xl relative overflow-hidden group">
        ...
        <div className="flex items-center justify-between relative z-10">
          <div className="flex items-center gap-4">
            <div className="p-3.5 bg-slate-800 rounded-xl text-amber-400 shadow-inner border border-slate-700">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M15.54 8.46a5 5 0 0 1 0 7.07"></path><path d="M19.07 4.93a10 10 0 0 1 0 14.14"></path></svg>
            </div>
            <div>
              <h3 className="font-extrabold text-white text-base tracking-tight">Order Alert Sound Mode</h3>
              <p className="text-xs text-slate-400 mt-0.5">Select how you want to be notified of incoming orders on this device.</p>
            </div>
          </div>
          <span className="px-3 py-1 bg-emerald-500/20 border border-emerald-500/40 text-emerald-400 text-[11px] font-black uppercase tracking-wider rounded-full shadow-[0_0_10px_rgba(16,185,129,0.2)]">
            Active
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
          <button
            ...
            className={cn(
              "p-4 rounded-xl border text-sm font-bold flex flex-col gap-2 transition-all cursor-pointer relative overflow-hidden group",
              alertMode === "store" 
                ? "bg-amber-500/10 border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.15)] ring-1 ring-amber-500/50" 
                : "bg-slate-800/50 border-slate-700 hover:border-slate-500 hover:bg-slate-800"
            )}
          >
            {alertMode === "store" && <div className="absolute top-3 right-3 text-amber-500"><CheckCircle2 className="w-5 h-5" /></div>}
            <div className={cn("text-xl mb-1", alertMode === "store" ? "opacity-100" : "opacity-60")}>🏪</div>
            <div className={cn("text-left", alertMode === "store" ? "text-amber-50" : "text-slate-300")}>Store Terminal Mode</div>
            <div className="text-[11px] font-medium text-slate-400 text-left font-normal leading-snug">
              Continuous loud alarm with full-screen takeover. Best for tablets or store counters.
            </div>
          </button>
          ...
          >
            {alertMode === "mobile" && <div className="absolute top-3 right-3 text-amber-500"><CheckCircle2 className="w-5 h-5" /></div>}
            <div className={cn("text-xl mb-1", alertMode === "mobile" ? "opacity-100" : "opacity-60")}>📱</div>
            <div className={cn("text-left", alertMode === "mobile" ? "text-amber-50" : "text-slate-300")}>Owner Mobile Mode</div>
            <div className="text-[11px] font-medium text-slate-400 text-left font-normal leading-snug">
              Single elegant chime with notification banner. Best for personal phones and silent use.
            </div>
          </button>
        </div>
      </div>
