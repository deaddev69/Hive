          ...
          <div className="lg:col-span-4 space-y-4 hidden lg:block">
            ...
            <button
              ...
              disabled={!selectedAddress || !isServiceable || !isAddressComplete(selectedAddress) || !dbBoutiques}
                ...
                className="w-full h-14 bg-hive-cream text-hive-dark hover:bg-hive-cream/80 border-2 border-hive-dark active:scale-[0.98] transition-all rounded-lg mt-3 font-semibold uppercase tracking-[0.2em] text-xs flex items-center justify-center gap-2 shadow-sm focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {!dbBoutiques ? (
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[999] bg-white border-t border-hive-border/30 shadow-[0_-8px_30px_rgb(0,0,0,0.06)] lg:hidden pb-[env(safe-area-inset-bottom)]">
        ...
        <div className="flex items-center justify-between px-5 py-4 gap-4">
          ...
          <button
            ...
            disabled={!selectedAddress || !isServiceable || !isAddressComplete(selectedAddress) || !dbBoutiques}
            onClick={() => router.push("/checkout/review")}
            className="flex-1 max-w-[200px] h-14 bg-hive-cream text-hive-dark hover:bg-hive-cream/80 border-2 border-hive-dark active:scale-[0.98] transition-all rounded-lg font-semibold uppercase tracking-[0.2em] text-xs flex items-center justify-center gap-1.5 shadow-sm focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {!dbBoutiques ? (
              <Loader2 className="w-4 h-4 animate-spin text-hive-dark" />
            ) : (
