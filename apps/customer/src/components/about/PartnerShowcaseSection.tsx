...
export function PartnerShowcaseSection() {
  ...
  return (
    <section className="relative w-full py-16 sm:py-24 overflow-hidden bg-neutral-950 text-white rounded-3xl border border-stone-800 shadow-2xl my-16 sm:my-20">
      {/* Editorial Decorative Ambience */}
      <div className="pointer-events-none absolute -top-24 -left-24 w-96 h-96 rounded-full bg-amber-500/10 blur-3xl" />
      <div className="pointer-events-none absolute -bottom-24 -right-24 w-96 h-96 rounded-full bg-stone-700/20 blur-3xl" />

      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 mb-10 sm:mb-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/10 pb-8">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-amber-300 text-[11px] font-mono font-semibold uppercase tracking-widest">
          ...
          <Link
            href="/products"
            className="inline-flex items-center gap-2 text-xs sm:text-sm font-semibold text-amber-300 hover:text-white transition-colors group shrink-0"
          >
        ...
        <Marquee speedSeconds={38} pauseOnHover={true} gap="1.5rem" maskGradients={true}>
          {brands.map((brand: any) => (
            <Link
              ...
              className="block outline-none focus-visible:ring-2 focus-visible:ring-amber-400 rounded-2xl sm:rounded-3xl"
              tabIndex={0}
      ...
      {/* Footer Sub-Note */}
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 mt-8 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-stone-400 font-medium">
